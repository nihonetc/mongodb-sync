...
[[ ( -n "${MONGODB_RESTORE_DB}" ) ]] && DB_RESTORE_STR=" --nsInclude=${MONGODB_RESTORE_DB}"
...
if mongorestore --drop --host ${MONGODB_RESTORE_HOST} --port ${MONGODB_RESTORE_PORT} --authenticationDatabase admin ${USER_RESTORE_STR}${PASS_RESTORE_STR} /backup/$FILE_TO_RESTORE; then
